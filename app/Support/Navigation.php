...
class Navigation
{
    ...
    public function endOfPeriod(Carbon $end, string $repeatFreq): Carbon
    {
        ...
        if (in_array($repeatFreq, ['FYC', 'FYP', 'FY3'], true) && $end->isFuture()) {
            // fall back to a yearly schedule if the requested period is a financial year.
            Log::debug(sprintf('endOfPeriod() requests "%s" + future, set it to "1Y" instead.', $repeatFreq));
            $repeatFreq = '1Y';
        }
    ...
    public function periodShow(Carbon $theDate, string $repeatFrequency): string
    {
        ...
        $formatMap = [
            ...
            'FYC'     => (string)trans('config.year_js'),
            'FYP'     => (string)trans('config.year_js'),
            'FY3'     => (string)trans('config.year_js'),
        ];
    ...
    {
        $previousFYDate = $date->copy()->subYear();
        /** @var FiscalHelperInterface $fiscalHelper */
        $fiscalHelper = app(FiscalHelperInterface::class);
        return $fiscalHelper->startOfFiscalYear($previousFYDate);
    }
    ...
    private function getPreviousFinancialYearEnd(Carbon $date): Carbon
    {
        ...
        /** @var FiscalHelperInterface $fiscalHelper */
        $fiscalHelper = app(FiscalHelperInterface::class);
        return $fiscalHelper->endOfFiscalYear($previousFYDate);
    }
    ...
    {
        $thirdLastFYDate = $date->copy()->subYears(2);
        /** @var FiscalHelperInterface $fiscalHelper */
        $fiscalHelper = app(FiscalHelperInterface::class);
        return $fiscalHelper->startOfFiscalYear($thirdLastFYDate);
    }
